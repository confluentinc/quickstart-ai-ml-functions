"""
Register the Confluent Cloud MCP server with Claude Code or Codex CLI from
Terraform core outputs.

After `uv run deploy`, run `uv run setup-mcp` to make this project's Confluent
Cloud cluster + Flink compute pool queryable directly from your coding agent via
the `confluent-cloud-mcp-server` (Node.js npx package `@confluentinc/mcp-confluent`).
...
Restart your coding agent after running this script.
...
def _node_bin_dir(npx_bin: str) -> str | None:
    """Return the directory containing the Node 24 `node`/`npx` we resolved.

    None when `_resolve_npx()` fell back to bare 'npx' (Node 24 already on PATH).
    """
    if npx_bin == "npx":
        return None
    return str(Path(npx_bin).parent)


def _build_mcp_path(npx_bin: str) -> str:
    """PATH for the MCP server process, with the Node 24 bin dir first.

    The @confluentinc/mcp-confluent and npx executables both start with
    `#!/usr/bin/env node`, so the child process re-resolves `node` from PATH.
    If the user's default `node` is newer than 24 (e.g. Node 26, ABI 147), the
    server loads @confluentinc/kafka-javascript under an ABI with no prebuilt
    binary and crashes with "Could not locate the bindings file" — Claude Code
    then silently drops the server. Pinning the Node 24 bin dir at the front of
    PATH makes every `env node` in the chain resolve to ABI 137, which has a
    prebuilt binary. Registering an absolute npx path alone is NOT enough; the
    shebang defeats it.
    """
    base = "/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin"
    node_dir = _node_bin_dir(npx_bin)
    return f"{node_dir}:{base}" if node_dir else base


def _clear_broken_npx_cache(npx_bin: str) -> None:
    """Remove cached @confluentinc/mcp-confluent entries whose native binary is
    missing or unloadable by the Node 24 we'll run the server with.

    Leaves a good cache in place (avoids a needless multi-minute rebuild) but
    forces a clean re-download when the kafka-javascript binding is absent or
    was built for the wrong ABI.
    """
    npx_cache = Path.home() / ".npm" / "_npx"
    if not npx_cache.exists():
        return
    node_bin = str(Path(npx_bin).parent / "node") if npx_bin != "npx" else "node"
    for entry in npx_cache.iterdir():
        if not entry.is_dir():
            continue
        kjs = entry / "node_modules" / "@confluentinc" / "kafka-javascript"
        if not (entry / "node_modules" / "@confluentinc" / "mcp-confluent").exists():
            continue
        binaries = list((kjs / "build" / "Release").glob("*.node")) if kjs.exists() else []
        good = False
        if binaries:
            try:
                subprocess.run(
                    [node_bin, "-e", f"require({str(binaries[0])!r})"],
                    capture_output=True,
                    check=True,
                    timeout=10,
                )
                ...
                good = False
        if not good:
            reason = "missing native binary" if not binaries else "binary built for wrong ABI"
...
def _register_with_claude_code(env_vars: dict, npx_bin: str, project_root: Path) -> None:
...
def _write_codex_config(config_path: Path, env_vars: dict, npx_bin: str) -> None:
    ...
    if config_path.exists():
        try:
            ...
            print(f"  Warning: could not parse existing config.toml ({exc}); preserving as .bak")
...
def _codex_config_paths(project_root: Path) -> list[Path]:
    ...
    if project_config_path.exists() and (project_config_path.resolve() != home_config_path.resolve()):
...
def _register_with_codex(env_vars: dict, npx_bin: str, project_root: Path) -> None:
    ...
    print(f"✓ Confluent MCP server registered as 'confluent-cloud-mcp-server' in {config_paths[0]}")
...
def main() -> None:
    ...
    # Pin the Node 24 bin dir first on the server's PATH so every `env node` in
    # the npx / mcp-confluent shebang chain uses ABI 137 (see _build_mcp_path).
    env_vars["PATH"] = _build_mcp_path(npx_bin)
